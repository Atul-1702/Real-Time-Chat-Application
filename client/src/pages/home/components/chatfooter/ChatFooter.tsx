...
function ChatFooter({ getAllMessage }) {
  ...
  const { selectedUser, user, userAllChats } = useSelector(
    (state) => state.userReducer
  );
  const socket = useSocket();
  const chat = userAllChats.find((item) => {
    if (
      item.members[0]._id === selectedUser ||
      item.members[1]._id === selectedUser
    ) {
      return true;
    }
  });
  ...
  return (
    <footer className="chat-footer-component-wrapper">
      <input ref={inputRef} type="text" placeholder="Type a message..." />
      <button onClick={onSendMessageButtonClicked}>
