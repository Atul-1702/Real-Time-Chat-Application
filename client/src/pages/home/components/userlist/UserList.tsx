  ...
  useEffect(() => {
    (async function () {
      ...
      if (response.success === true) {
        ...
        if (searchedUser) {
          setAllSearchedUser(
            (response.data as User[]).filter((user: User) => {
              if (
                user.firstname
                  .toLowerCase()
                  .includes(searchedUser.toLowerCase()) === true ||
                user.lastname
                  ?.toLowerCase()
  ...
  async function onToggleChatArea(userId: string) {
    ...
    for (const item of userAllChats) {
      if (
        ...
        (item?.members[0]?._id == userId || item?.members[1]?._id == userId)
      ) {
    ...
    <ul className="user-list-container">
      {allSearchedUser.map((user: User) => (
        <li
          key={user.createdAt}
          className={
            selectedUser === user._id ? "selected-item" : "user-list-item"
          }
          onClick={() => onToggleChatArea(user._id)}
        >
          {!user.profilepic ? (
            <span className="name-intials">
          ...
          {userAllChats.some((item) =>
            item?.members.map((m) => m._id).includes(user._id)
          ) ? (
            ...
            <button onClick={() => onStartChatButtonClicked(user._id)}>
              Start Chat
            </button>
          )}
        </li>
